...
Codex Desktop 会把本地聊天记录、项目侧边栏、账号状态等数据保存在本机的 `.codex` 目录里。  
当你从官方账号登录切换到 CPAMC/API 接入，或者从 CPAMC/API 切回官方 OAuth 登录时，聊天记录并不一定真的丢失，但 Codex 可能因为 provider 不同而不显示原来的对话。
